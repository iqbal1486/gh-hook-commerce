...
class ABRAC_AjaxCall
{

    public function __construct()
    {
        add_action("wp_ajax_abrac_conversion_call", array(
            $this,
            "abrac_conversion_call"
        ));
        add_action("wp_ajax_nopriv_abrac_conversion_call", array(
            $this,
            "abrac_conversion_call"
        ));

        add_action("wp_ajax_abrac_listing_call", array(
            $this,
            "abrac_listing_call"
        ));
        add_action("wp_ajax_nopriv_abrac_listing_call", array(
            $this,
            "abrac_listing_call"
        ));
    ...
    public function abrac_listing_call()
    {
        ...
        if ($transak_payment_supported)
        {
            ...
            /**TEST*
            $transak_html .= "<pre>";
            $transak_html .= print_r($get_transak_currencies_quote, true);
            $transak_html .= "</pre>";
            ...
            if( $show_transak ){
                ...
                if ($get_transak_currencies_quote->error || !$transak_country_supported || !$transak_state_supported || !$countries_payment_method_supported)
                {
                    ...
                    $transak_html .= "<div class='provider'><img src='" . ABRAC_PLUGIN_URL . "/assets/images/transak.png' /></div>";
                    $transak_html .= "</div>";
                    ...
                    foreach ($get_payment_method as $key => $value)
                    {
                        ...
                        {
                            $transak_html .= "<img src='" . ABRAC_PLUGIN_URL . "/assets/images/" . $value['payment_method'] . ".svg' />";
                        }
                    }
                    $transak_html .= "</div>";
                    $transak_html .= "</div>";
                ...
                else
                {
                    ...
                    //$transak_html .= "<div class='estimate-block'> <span> 1 ".$crypto." ~ ". round($currencies_exchange_rate + ($currencies_exchange_rate * ($transak_interest - 1)), 2) ."</span>  <span> ".$currency."</span></div>";
                    $transak_html .= "</div>";

                    $transak_payment_page = get_the_permalink(get_option('abrac_transak_payment_page'));
                    $transak_payment_page = MOONPAY_THIRD_PARTY_PAGE;

                    $transak_html .= "<div class='right-half-right'>";
                    //$html .= "<form method='get' action='".$transak_payment_page."'>";
                    ...
                    $transak_html .= '<label for="transak_acceptance" class="transak_acceptance">
                        ...
                        <span class="transak_acceptance_text">
                          <span>I agree with&nbsp;<a href="' . get_the_permalink(ABRAC_TERMS_URL) . '" target="_blank">Terms of Use</a> and &nbsp;<a href="' . get_the_permalink(ABRAC_PRIVACY_URL) . '" target="_blank">Privacy Policy</a></span>
                        </span>
                      </label>';

                    $transak_html .= "<input name='abrac_buy_now' type='hidden' value='" . wp_create_nonce('abrac-buy-now') . "'>";
    ...
    // define the function to be fired for logged in users
    public function abrac_conversion_call()
    {
    ...
    public function get_transak_currencies_quote($cryptocurrencyCode, $baseCurrencyAmount, $baseCurrencyCode)
    {
        ...
        $apiUrl = ABRAC_TRANSAK_CURRENCY_API . '?fiatCurrency=' . strtoupper($baseCurrencyCode) . '&cryptoCurrency=' . strtoupper($cryptocurrencyCode) . '&isBuyOrSell=BUY&fiatAmount=' . $baseCurrencyAmount;
    ...
    public function get_transak_currencies_exchange_rate($cryptocurrencyCode, $baseCurrencyAmount, $baseCurrencyCode)
    {
        ...
        $apiUrl = ABRAC_TRANSAK_CURRENCY_API . '?fiatCurrency=' . strtoupper($baseCurrencyCode) . '&cryptoCurrency=' . strtoupper($cryptocurrencyCode) . '&isBuyOrSell=BUY&cryptoAmount=1';
        $response = wp_remote_get($apiUrl);
        $responseBody = wp_remote_retrieve_body($response);
        $result = json_decode($responseBody);
        if ($result && !is_wp_error($result))
...
$abrac_ajax_call = new ABRAC_AjaxCall;